...
type keystoreDetector struct{}

func NewKeystoreDetector() keystoreDetector {
	return keystoreDetector{}
}

func (keystoreDetector) Detect(location string) (Keystore, error) {
	...
	if len(buf) > 3 && buf[0] == 0xFE && buf[1] == 0xED &&
		buf[2] == 0xFE && buf[3] == 0xED {
